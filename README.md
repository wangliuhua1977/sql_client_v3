...
## 主要功能
- **多窗口/面板 SQL 笔记**：默认 `JDesktopPane + JInternalFrame` 子窗口，可切换到“视图-面板模式”使用顶部标签在单窗体内切换编辑器；子窗口支持拖动、最大化、最小化、调整大小与双击重命名，窗口可平铺排列。
...
- **可配置输入修正**：在“文件-设置”中可开启/关闭中文全角符号自动转半角（默认开启），避免中文标点导致 SQL 语法错误。
...
## 版本更新日志
- **v0.3.13**
  - 面板模式切换到独立窗口模式时会完整恢复所有已打开的笔记窗口，而不再只打开首个窗口。
  - “文件-设置”新增 SQL 编辑器选项，可控制全角符号自动转换，默认开启以避免中文标点导致的编译错误。
  - 别名列联想顶部新增“所有字段”条目，选择后按当前别名批量插入逗号分隔的字段列表，继续按使用次数排序并展示引用计数。
