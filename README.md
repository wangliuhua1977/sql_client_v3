...
- 快捷键表：
  ...
  | Ctrl+L | 聚焦编辑器输入区 |
  | Ctrl+F | 打开查找/替换条并聚焦查找框 |
  | F3 / Shift+F3 | 查找下一处 / 上一处 |
  | Esc | 停止/取消正在执行的任务 |
...
## 核心操作
### SQL 编辑器查找/替换
- **激活**：编辑器聚焦时按 `Ctrl+F` 打开查找条，光标自动进入“查找”输入框；若已有选中文本且查找框为空，会自动填入并立即高亮匹配。
- **导航**：在查找框内 `Enter`=下一处、`Shift+Enter`=上一处；编辑器内 `F3`=下一处、`Shift+F3`=上一处。
- **关闭**：`Esc` 关闭查找条并回到编辑器光标位置（仅在查找条显示时生效）。
- **替换**：点击“替换”展开替换行，支持“替换/替换并查找/全部替换”，全部替换完成后状态区提示“已替换 N 处”。
- **选项**：
  - 区分大小写、全词匹配、正则、循环查找（Wrap）。
  - 全词匹配规则：字母/数字/下划线视为词字符，匹配需满足左右边界非词字符或文本边界。
  - 正则匹配基于 `Pattern`，语法错误会在状态区提示“正则错误：...”，不抛异常到 UI。
- **高亮策略**：默认高亮全部匹配；当匹配数超过阈值（当前为 800）时仅高亮当前匹配并提示“匹配过多，仅定位当前”。编辑器文本变化使用 200ms 防抖刷新，避免大文本卡顿。

### 开发说明（查找/替换）
- 统一入口：`FindReplaceSupport.install(JTextComponent editor, JComponent host)`，由 `EditorTabPanel` 在创建 SQL 编辑器时安装，保证主编辑区、临时笔记窗、函数/存储过程源码窗一致可用。
- 核心类职责：
  - `FindReplaceEngine`：纯逻辑查找引擎（正则/大小写/全词/方向/循环），可独立测试。
  - `FindReplacePanel`：查找/替换 UI（输入框、按钮、选项、状态区）。
  - `FindReplaceController`：UI 与引擎桥接，负责高亮、输入监听、替换与快捷键。
  - `FindOptions` / `FindResult`：不可变参数与查找结果模型。
- 扩展点：若新增 SQL 编辑窗口，仅需在编辑器装配完成后调用 `FindReplaceSupport.install` 即可复用；可在 `FindReplaceController` 中调整高亮阈值、刷新防抖时间、状态提示文案。
### 函数/存储过程源码编辑与调试运行
...
### 元数据刷新（并发与审计日志）
- 全量刷新采用 **每页 200 条** 分段拉取，并使用 **4 线程** 并行分页查询，按 `schema_name + object_name` 去重，避免重复与遗漏。
- 稽核机制：拉取完成后对比远端计数签名，若发现数量不一致或重复数据，将记录错误并终止本次刷新，避免覆盖本地缓存。
- 本地文本日志：所有元数据拉取与审计事件会写入可配置的日志文件，包含时间戳、线程、操作类型、状态与错误信息。
  - `metadata.log.path`：日志文件路径，默认 `~/.Sql_client_v3/logs/metadata-sync.log`。
  - `metadata.log.format`：日志格式模板，默认 `{time} [{thread}] {type} {status} {message}`，支持占位符 `{time}` `{thread}` `{type}` `{status}` `{message}`。
