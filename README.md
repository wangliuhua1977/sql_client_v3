...
-## 版本更新日志
- **v0.3.40**
  - SQL 执行保持原始语句并用 `$$...$$` 包裹后作为查询参数，发送前统一进行 URL 编码，保证包含换行/空格/中文的语句也能合法提交而不报 URI 异常。

- **v0.3.39**
  - 保障日志工具类随源代码一同编译，SQL 执行服务在记录前先检测日志面板是否就绪，避免找不到 `OperationLog` 导致的编译或运行异常。

- **v0.3.38**
  - 主窗体右侧新增“操作日志”面板，实时记录控件变化、SQL/元数据 HTTPS 原始请求与响应，可清空历史。
  - SQL 执行与元数据抓取均输出详细日志，便于排查非 JSON 返回导致的执行提示。

- **v0.3.37**
  - SQL 执行改为保持原始语句并以 `$$...$$` 包裹后直接提交，不再进行转码或引号替换，确保所有 DDL/DML 均按原文发送。

- **v0.3.36**
  - SQL 执行请求直接 URL 编码原始语句，不再全局替换单引号，避免 `CREATE TABLE AS SELECT` 等包含字符串字面量的语句被破坏。

- **v0.3.35**
  - 数据集面板在主窗体和子窗体内均默认隐藏，仅当有查询结果时自动展开；若无返回数据保持收起状态。
  - 工具栏执行/停止按钮随当前焦点窗口的执行态切换：可用时蓝色边框+图标，不可用时黑色边框+灰色图标，运行中禁用执行、启用停止。

- **v0.3.34**
  - 工具菜单“重置元数据”会提醒清空本地表/函数/存储过程及字段缓存（包含使用频次），确认后先删除缓存再重新拉取一次对象列表。

- **v0.3.33**
  - SQL 执行前将单引号替换为两个单引号后再编码调用接口，避免后端解析异常。
  - 面板模式统一使用底部共享结果面板，所有 SQL 结果以选项卡切换；结果集面板默认收起，有返回数据时自适应展开。
  - 结果集标题不再显示行数，表格上方显示 “记录数 X 条”，列顺序按照接口返回的 sort_no 排序。
- **v0.3.32**
  - HTTPS 执行 SQL 前自动将单引号转换为双引号再编码，避免接口因引号报错导致执行失败。
- **v0.3.31**
  - SQL 执行响应解析加固，包含单引号等字符的语句返回异常时不再触发空指针，界面会给出友好错误提示。
  - 移除结果区折叠按钮，改为仅靠分割条上下拖动调整编辑器与结果表格高度，分割条保持合并状态（不可分离/重叠），空结果时默认收起下方区域。
- **v0.3.30**
  - 结果折叠控件改为上下箭头指示，移除“结果面板”文字按钮，窗口模式与共享结果视图保持一致。
  - 结果区保持组件常驻，SQL 编辑器下缘可直接拖拽分割条调整高度，拖动或箭头切换时同步上移数据集面板并记忆上次高度。
- **v0.3.29**
  - SQL 编辑器与结果区之间增加可拖拽分割条，下缘可上下拉伸；结果折叠按钮改为上下箭头且默认收起，展开后保留上次高度。
  - 子窗口状态栏展示执行动画，执行中禁用当前窗口的“开始”按钮，切换焦点时主窗体的开始/停止状态与当前窗口运行态同步。
  - 结果表格使用连续实线网格与浅色表头，结果选项卡/共享结果折叠钮统一为箭头提示。
- **v0.3.28**
