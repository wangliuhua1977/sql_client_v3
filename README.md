...
### OptionalInt 转 Integer 与 primitive 安全准则
- 解析 `CommandTag` 时若得到 `OptionalInt`，统一写法：
  ```java
  OptionalInt parsed = CommandTagParser.parseAffectedRows(commandTag);
  Integer affectedRows = parsed.isPresent() ? parsed.getAsInt() : null;
  ```
  禁止使用 `.orElse(null)` 以免引发 `<nulltype> cannot be converted to int` 类编译错误。
- 禁止将 `null` 传入 primitive 形参或自动拆箱为 primitive；所有可空数值统一使用包装类型（Integer/Long/Boolean 等），或在拆箱前通过三元表达式/`Objects.requireNonNullElse` 提供显式默认值。
