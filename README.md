...
## UI 设计与交互（科技感极简版）
- **主布局**：左侧对象树，中央多标签编辑器（独立窗口/面板两种模式），底部共享结果与消息区，底部状态栏提示执行、自动保存、后台任务与元数据状态，双层 SplitPane 保留 8px 分隔线。
- **菜单结构**：File / Edit / Run / View / Tools / Help。高频动作（执行、停止、切换标签、聚焦编辑器）在 Run 菜单与快捷键中呈现，低频工具（元数据刷新、SQL 片段库、执行历史、备份恢复等）集中在 Tools，布局与停靠切换集中在 View。
- **隐藏策略**：日志、历史、设置等辅助面板默认隐藏，仅可通过 View 菜单复选框显式展开；右侧辅助区域初始不可见，底部共享结果区在有结果时自动展开。
- **快捷键一览**：
  - Ctrl+Enter 执行当前 SQL，Esc 停止/取消执行。
  - Ctrl+W 关闭当前标签，Ctrl+Tab / Ctrl+Shift+Tab 切换标签，Ctrl+L 聚焦编辑器，F6 在对象树/编辑器/结果区之间循环焦点。
  - Alt+E 切换联想、Alt+W 打开 SQL 片段库、Alt+Q 打开执行历史。
- **渲染与间距**：全局字体 12px，表格行高 24px，树行高 22px；外框采用 12px 留白 + 1px Matte 分隔线，去除厚边框与标题。JTree 使用轻量渲染器区分类别/对象层级，JTable 采用弱化网格、柔和选中背景、`null` 空显示，表头加粗并收敛边框。
