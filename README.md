...
## 表格导入到 PostgreSQL（leshan）
- 入口：菜单 Tools -> “表格导入到PG(粘贴/CSV/XLSX)...”，快捷键 Windows/Linux `Ctrl+Shift+I`，macOS `⌘+Shift+I`。
- 独立向导窗口（非模态），使用 CardLayout 包含 4 步：选择数据源、设置并读取、预览/模式选择、执行与报告。
- 支持数据源：粘贴区域（含标题行）、CSV 文件（编码/分隔符可选）、XLSX 文件（可选 Sheet 与起始行），读取时严格保持列对齐并提供前 50 行预览。
- 写入模式：新建表、TRUNCATE 后插入、追加插入；导入过程使用后台线程+有界队列分批写入，支持取消并在 UI 日志区显示进度。
- 导入结束会生成文本报告，可在向导中复制或导出为 txt 文件，包含源/表/批次/行数等摘要。
