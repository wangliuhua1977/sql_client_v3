...
## 主要功能
- **多标签 SQL 编辑**：行号、语法高亮、自动缩进，支持 PostgreSQL 与 Hive 标签独立保存。
- **文件操作**：文件菜单内提供新建、打开、保存当前、保存全部，打开时可选择数据库类型，保存支持选择路径并在后台自动保存。
- **自动保存**：每个标签独立后台线程定期写盘，状态栏显示最近保存时间。
- **智能联想**：仅在表/视图、字段、函数/存储过程位置弹出；支持 Ctrl+Space 开关、上下键与回车选择、双击选择、使用频次排序与高亮，并记录使用次数。
- **别名与模糊匹配**：`FROM/JOINS` 中的别名可用于列联想，支持 `%` 分词模糊匹配与下划线连续输入不关闭联想框。
