...
## UI 设计与交互
- **主布局**：左侧为对象树（可按关键字过滤），中部为 SQL 编辑器，多标签/子窗体双模式，底部为结果区，最下方是轻量状态栏；左右、上下均采用 SplitPane，可拖拽或在“视图”菜单中一键隐藏/显示。
- **菜单体系**：
  - File/编辑：新建/导入/保存笔记，关闭当前标签（Ctrl+W）。
  - Run：执行（Ctrl+Enter）、停止（Esc）、聚焦编辑器（Ctrl+L）。
  - View：显示/隐藏对象树、结果区、日志/历史/设置附加面板，日志停靠位置切换，主题与布局重置。
  - Window/Tools/Help：标签切换、平铺窗口、元数据工具、SQL 片段库/执行历史、快捷键一览/使用说明。
- **辅助面板默认隐藏**：日志、历史、设置等均收纳在右侧辅助区，初始不可见，仅通过 View 菜单显式开启；开启后仍可随时收起或切换停靠位置。
- **快捷键表**：
  - Ctrl+Enter 执行当前 SQL
  - Esc 停止/取消执行
  - Ctrl+W 关闭当前标签
  ...
  - Ctrl+L 聚焦编辑器
  - F6 在对象树/编辑器/结果之间循环焦点
  - Alt+E 切换联想、Alt+W 打开 SQL 片段库、Alt+Q 打开执行历史
- **视觉/渲染器**：全局字体 12 号、表格行高 24/树行高 22，轻量 EmptyBorder + MatteBorder 控制 8/12 留白；对象树 renderer 对根/分组加粗并柔和选中底色；结果表格使用统一 renderer（null 显示为空，弱化网格、选中底色提升对比，表头加粗）；UIManager 统一焦点宽度与表格行高，保持科技感极简风格。
