...
- 快捷键表：
  ...
  | Ctrl+Shift+I / ⌘+Shift+I | 打开“Excel 粘贴导入到PG…”向导 |
...
### Excel 粘贴导入到 PostgreSQL（leshan）
- 入口：工具菜单新增“Excel 粘贴导入到PG…”，快捷键 `Ctrl+Shift+I`（macOS 为 `⌘+Shift+I`），打开独立向导窗口。
- 向导步骤：
  1. 粘贴：在文本框中直接粘贴从 Excel/WPS 复制的表格（含标题），解析失败会提示具体行列不一致。
  2. 预览与配置：自动推断字段名与 PG 类型（boolean/integer/bigint/numeric/date/timestamp/text），可手工调整；默认 schema `leshan`，表名形如 `paste_yyyyMMdd_HHmmss` 可编辑，支持空字符串视为 NULL 开关，写入模式包括新建表、清空并插入、追加插入（可选去重列）。
  3. 执行：显示概要、进度条与日志，后台线程提交建表/插入 SQL，失败会回显数据库原始错误信息。
- 严格一致性：解析时要求所有数据行列数与标题列完全一致，禁止自动补空或截断；INSERT 采用显式列清单，批量入库时保留原始行序与列顺序。
